...
tmap::tm_shape(dat_gdp) + 
  ...
  tmap::tm_borders(lwd=0.1, alpha=0.1) + 
...
The summary of `nb_q` reveals that the average number of neighbours (adjacent polygons) is 5.9, 14 polygons have only 1 neighbour and the most connected municipality borders 23 neighbours. 2 municipalities (1523 and 3497) are islands and hence, according to this definition, have no neighbours. Since they are only of minor importance and not related to any mining activities at all, I exclude them from the data.
...
plot(nb_q, coords=st_coordinates(coords), add=TRUE, points=FALSE, lwd=0.3)
```
...
plot(sf::st_geometry(base_mun), border="grey", lwd=0.5)
plot(nb_5nnb, coords=st_coordinates(coords), add=TRUE, points=FALSE, lwd=0.3)
```
...
plot(sf::st_geometry(base_mun), border="grey", lwd=0.5)
plot(nb_dist, coords=st_coordinates(coords), add=TRUE, points=FALSE, lwd=0.3)
```
